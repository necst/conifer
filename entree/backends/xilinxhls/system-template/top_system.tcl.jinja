...
for {set i 0} {$i < $n_banks} {incr i} {
    for {set j 0} {$j < $n_trees_per_bank} {incr j} {	
		...
		set_property -dict [list CONFIG.ENABLE_DFX {true}] [get_bd_cells tree_rm_${i_class}_${j_tree_in_class}]	
		set_property name tree_rp_${i}_${j} [get_bd_cells tree_rm_${i_class}_${j_tree_in_class}]
		
		if {$j_tree_in_class < [expr { $n_trees_per_class - 1 }] } {
